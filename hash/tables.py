"""
tables.py — константы алгоритма ГОСТ Р 34.11-2012 («Стрибог»).

Содержит константы алгоритма ГОСТ Р 34.11-2012 и проверки их целостности:
  - PI      — нелинейная подстановка π
  - TAU     — перестановка байт τ
  - MATRIX_A — матрица линейного преобразования A
  - C       — 12 итерационных констант C1..C12
  - IV_512  — инициализационный вектор для хэша длиной 512 бит
  - IV_256  — инициализационный вектор для хэша длиной 256 бит

Соглашение об endianness (действует во всем проекте):
  state — bytes длиной 64
  state[0] — GOST-байт a_0 (младший, «правый» в записи ГОСТа)
  state[63] — GOST-байт a_63 (старший, «левый» в записи ГОСТа)
  display hex: state[::-1].hex()   ← то, что напечатано в стандарте
  internal ← read: bytes.fromhex(s)[::-1]
"""


# ---------------------------------------------------------------------------
# 5.2  Нелинейная подстановка π : V_8 → V_8
# ---------------------------------------------------------------------------
# PI[i] = π(i) для i ∈ {0..255}
# Источник: ГОСТ Р 34.11-2012, таблица раздела 5.2
...
# ---------------------------------------------------------------------------
# 5.3  Перестановка байт τ : {0..63} → {0..63}
# ---------------------------------------------------------------------------
# TAU[i] = τ(i) — из какой позиции берем байт при перестановке P
# Источник: ГОСТ Р 34.11-2012, таблица раздела 5.3
...
# ---------------------------------------------------------------------------
# 5.4  Матрица линейного преобразования A над GF(2)
# ---------------------------------------------------------------------------
# 64 строки; строка j хранится как 64-битное целое.
# Бит k строки j (считая от LSB) = элемент матрицы A[j][k].
#
# Применение к 64-битному слову w:
#   out = XOR{ MATRIX_A[63-k]  для каждого k, где (w >> k) & 1 == 1 }
# Такое «зеркалирование» индекса (63-k) объясняется тем, что в ГОСТе
# строки матрицы нумеруются от 0 снизу, а бит k слова w соответствует
# GOST-биту k, умножающемуся на строку с номером k.
#
# Источник: ГОСТ Р 34.11-2012, таблица раздела 5.4
...
# ---------------------------------------------------------------------------
# 5.5  Итерационные константы C1..C12
# ---------------------------------------------------------------------------
# Хранятся как целые числа в «display order» (старший байт слева,
# как напечатано в стандарте).
#
# Для использования в алгоритме нужно преобразовать в bytes:
#   C_BYTES[i] = C[i].to_bytes(64, 'little')
# (little-endian = display[::-1], соответствует нашему соглашению)
#
# Источник: ГОСТ Р 34.11-2012, раздел 5.5
...
# ---------------------------------------------------------------------------
# Хранятся во внутреннем формате проекта (bytes длиной 64):
#   IV_512 = 0^512  → все нули
#   IV_256 = (00000001)^64 → все байты равны 0x01
...
# ---------------------------------------------------------------------------
# Предвычисленные C_BYTES — константы в рабочем формате (little-endian bytes)
# ---------------------------------------------------------------------------
# Используются в streebog.py напрямую, чтобы не конвертировать на каждой итерации.

C_BYTES = tuple(c.to_bytes(64, 'little') for c in C)


# ---------------------------------------------------------------------------
# Проверки целостности таблиц (выполняются при импорте)
# ---------------------------------------------------------------------------

assert len(PI)       == 256,                 "PI: должно быть 256 элементов"
assert len(set(PI))  == 256,                 "PI: должна быть биекцией"
assert min(PI)       == 0,                   "PI: минимальное значение 0"
assert max(PI)       == 255,                 "PI: максимальное значение 255"

assert len(TAU)      == 64,                  "TAU: должно быть 64 элемента"
assert sorted(TAU)   == list(range(64)),     "TAU: должна быть биекцией"

assert len(MATRIX_A) == 64,                  "MATRIX_A: должно быть 64 строки"
assert all(0 <= x < (1 << 64) for x in MATRIX_A), "MATRIX_A: значения 64-битные"

assert len(C)        == 12,                  "C: должно быть 12 констант"
assert all(0 < x < (1 << 512) for x in C),  "C: значения 512-битные"
assert len(C_BYTES)  == 12,                  "C_BYTES: должно быть 12 элементов"
assert all(len(b) == 64 for b in C_BYTES),   "C_BYTES: каждый элемент 64 байта"

assert len(IV_512)   == 64,                  "IV_512: должен быть 64 байта"
assert len(IV_256)   == 64,                  "IV_256: должен быть 64 байта"
assert IV_512        == b'\x00' * 64,        "IV_512: все нули"
assert IV_256        == b'\x01' * 64,        "IV_256: все 0x01"
